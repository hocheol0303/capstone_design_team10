...
MODEL_PATH = "/home/work/hocheol_dir/workspace/save_models/age_cls/v0.2.3/250908_083716_S/epoch70_250908_083716.keras"
...
def make_gradcam_manual(img_array, model, feature_extractor, output_index=0):
    with tf.GradientTape() as tape:
        ...
        male_head = model.get_layer('dense')(x)
        male_output = model.get_layer('male')(male_head)

        female_head = model.get_layer('dense_1')(x)
        female_output = model.get_layer('female')(female_head)

        preds = [male_output, female_output]

        target_preds = preds[output_index]
        top_pred_index = tf.argmax(target_preds[0])
        top_class_channel = target_preds[:, top_pred_index]
    ...
    return heatmap.numpy(), int(top_pred_index.numpy())

def make_heatmap(heatmap, input_tensor, path, pred_class):
...
if __name__ == '__main__':
    ...
    for batch in tqdm(dataset):
        images, labels, weights, paths = batch
        
        if weights['male'][0].numpy() == 1.0:
            heatmap, pred_class = make_gradcam_manual(images, model, feature_extractor, output_index=0)
        else:
            heatmap, pred_class = make_gradcam_manual(images, model, feature_extractor, output_index=1)
        
        make_heatmap(heatmap, images, paths.numpy()[0].decode('utf-8'), pred_class)