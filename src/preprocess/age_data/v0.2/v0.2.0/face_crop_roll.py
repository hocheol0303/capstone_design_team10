...
output_folder = input_folder / "age_cropped"
...
def age_preprocess(source_path, dest_path):
    image = cv2.imread(source_path)
    if image is None:
        raise Exception(f"⚠️ Failed to load {source_path}")
    ...
    with mp.solutions.face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1, refine_landmarks=True) as face_mesh:
        ...
        if not results.multi_face_landmarks:
            raise Exception(f"❌ No face detected in {source_path}")
        ...
        if cv2.imwrite(dest_path, cropped):
            return None
        else:
            return source_path
