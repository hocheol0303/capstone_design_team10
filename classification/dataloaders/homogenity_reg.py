...
class DataLoader:
    def __init__(self, item_list, root_dir, config, is_training=True):
        ...
        for item in item_list:
            ...
            self.rad_labels.append(float(item['radiance_label']))
            self.tex_labels.append(float(item['texture_label']))
    ...
    def _preprocess(self, path, label):
        ...
        label['rad_out'] = tf.cast(label['rad_out'], tf.float32)
        label['tex_out'] = tf.cast(label['tex_out'], tf.float32)
