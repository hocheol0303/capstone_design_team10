...
class DataLoader:
    ...
    def _preprocess(self, path, label, gender):
        ...
        return img, {'male': label_float, 'female': label_float}, {'male': male_weight, 'female': female_weight}

    def _preprocess_with_mask(self, path, label, gender, forehead_box, right_eye_box, left_eye_box, nasolabial_box, oral_box):
        img = tf.io.read_file(path)
        img = tf.image.decode_jpeg(img, channels=3)

        if tf.random.uniform(()) < 0.5 and forehead_box[0] != -1:
            img = _mask_image(img, forehead_box)
        if tf.random.uniform(()) < 0.5:
            if right_eye_box[0] != -1:
                img = _mask_image(img, right_eye_box)
            if left_eye_box[0] != -1:
                img = _mask_image(img, left_eye_box)
        if tf.random.uniform(()) < 0.5 and nasolabial_box[0] != -1:
            img = _mask_image(img, nasolabial_box)
        if tf.random.uniform(()) < 0.5 and oral_box[0] != -1:
            img = _mask_image(img, oral_box)
        if tf.random.uniform(()) < 0.5:
